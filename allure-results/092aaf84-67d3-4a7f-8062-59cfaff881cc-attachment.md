...
- Location: tests\multiUserLogin.test.js:44:7
...
Call log:
  - waiting for locator('#logout2') to be visible
    34 × locator resolved to hidden <a href="#" id="logout2" class="nav-link" onclick="logOut()">Log out</a>
      ...
      - generic [ref=e4]:
        ...
        - generic [ref=e9]:
          - generic [ref=e10]:
            ...
            - textbox [ref=e12]: testuser3
          - generic [ref=e13]:
            - generic [ref=e14]: "Password:"
            - textbox [ref=e15]: password789
        - generic [ref=e17]:
...
```ts
  ...
  28 |     await login.navigate();   // ensure homepage loads
  29 | 
  30 |     const users = readCSV('data-files/users.csv');   // FIXED PATH
  31 |     for (const user of users) {
  32 |       await login.login(user.username, user.password);
  33 | 
  34 |       // Wait until login modal disappears and logout button is visible
  35 |       await page.waitForSelector('#logout2', { state: 'visible', timeout: 15000 });
  36 |       await expect(page.locator('#logout2')).toBeVisible();
  37 | 
  38 |       // Logout to reset state
  39 |       await page.click('#logout2');
  40 |       await page.waitForSelector('#login2', { state: 'visible', timeout: 10000 });
  41 |     }
  42 |   });
  43 | 
  44 |   test('Login with multiple users from JSON', async ({ page }) => {
  45 |     const login = new LoginPage(page);
  46 |     await login.navigate();   // ensure homepage loads
  47 | 
  48 |     const users = readJSON('data-files/users.json'); // FIXED PATH
  49 |     for (const user of users) {
  50 |       await login.login(user.username, user.password);
  51 | 
  52 |       // Wait until login modal disappears and logout button is visible
> 53 |       await page.waitForSelector('#logout2', { state: 'visible', timeout: 15000 });
     |                  ^ TimeoutError: page.waitForSelector: Timeout 15000ms exceeded.
  54 |       await expect(page.locator('#logout2')).toBeVisible();
  55 | 
  56 |       // Logout to reset state
  57 |       await page.click('#logout2');
  58 |       await page.waitForSelector('#login2', { state: 'visible', timeout: 10000 });
  59 |     }
  60 |   });
  61 | 
  62 | });
