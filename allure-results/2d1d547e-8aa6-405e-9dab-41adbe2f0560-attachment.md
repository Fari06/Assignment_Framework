...
Call log:
  - waiting for locator('#logout2') to be visible
    24 × locator resolved to hidden <a href="#" id="logout2" class="nav-link" onclick="logOut()">Log out</a>
...
```ts
  ...
  33 | 
  34 |       // Wait for logout button to appear
  35 |       await page.waitForSelector('#logout2', { state: 'visible', timeout: 10000 });
  ...
  51 | 
  52 |       // Wait for logout button to appear
> 53 |       await page.waitForSelector('#logout2', { state: 'visible', timeout: 10000 });
     |                  ^ TimeoutError: page.waitForSelector: Timeout 10000ms exceeded.
