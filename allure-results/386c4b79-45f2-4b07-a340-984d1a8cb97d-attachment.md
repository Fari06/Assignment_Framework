...
```ts
  ...
  8  | 
  9  |   // Navigate to home and add item
  10 |   await productPage.navigateToHome();
  11 |   await productPage.addItemToCart('Sony vaio i5');
  12 | 
  13 |   // Go to cart
  14 |   await page.click('#cartur');
  15 |   await page.waitForSelector('#tbodyid');
  16 | 
  17 |   // ✅ Remove item(s) from cart
> 18 |   await cart.removeAllItems();
     |              ^ TypeError: cart.removeAllItems is not a function
  19 | 
  20 |   // ✅ Verify cart is empty by checking row count
  21 |   const itemsCount = await page.locator('#tbodyid tr').count();
  22 |   expect(itemsCount).toBe(0);
  23 | 
  24 |   // Place order (optional, even with empty cart)
  25 |   await cart.placeOrder(
  26 |     'Farheen',
  27 |     'India',
  28 |     'Delhi',
  29 |     '1234567890123456',
  ...
  32 |   );
  33 | 
  34 |   // ✅ Confirmation check
  35 |   const confirmation = await page.locator('.sweet-alert').textContent();
  36 |   expect(confirmation).toContain('Thank you for your purchase!');
  37 | });
```