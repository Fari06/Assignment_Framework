...
- Name: login.test.js >> Valid login works
- Location: tests\login.test.js:4:5
...
```ts
  ...
  3  | 
  4  | test('Valid login works', async ({ page }) => {
  5  |   const login = new LoginPage(page);
  6  |   await login.navigate();
  7  |   await login.login('testuser1', 'Pass@123');
> 8  |   await expect(page.locator('#logout2')).toBeVisible();
     |                                          ^ Error: expect(locator).toBeVisible() failed
  9  | });
  10 | 
  11 | test('Invalid login fails', async ({ page }) => {
  12 |   const login = new LoginPage(page);
  13 |   await login.navigate();
  14 |   await login.login('wronguser', 'wrongpass');
  15 |   const alertText = await page.on('dialog', dialog => dialog.message());
  16 |   expect(alertText).toContain('User does not exist');
  17 | });
```