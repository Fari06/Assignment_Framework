...
```ts
  ...
  28 |     await login.navigate();
  29 | 
  30 |     const users = readCSV('data-files/users.csv');
  31 |     for (const user of users) {
  32 |       await login.login(user.username, user.password);
  33 | 
  34 |       // ✅ Check login success by "Welcome <username>" text
> 35 |       await expect(page.locator('#nameofuser')).toContainText(user.username, { timeout: 10000 });
     |                                                 ^ Error: expect(locator).toContainText(expected) failed
  36 | 
  ...
  45 |     await login.navigate();
  46 | 
  47 |     const users = readJSON('data-files/users.json');
  48 |     for (const user of users) {
  49 |       await login.login(user.username, user.password);
  50 | 
  51 |       // ✅ Check login success by "Welcome <username>" text
  52 |       await expect(page.locator('#nameofuser')).toContainText(user.username, { timeout: 10000 });
  53 | 
