...
- Location: tests\multiUserLogin.test.js:43:7
...
```ts
  ...
  28 |     await login.navigate();   // homepage load
  29 | 
  30 |     const users = readCSV('data-files/users.csv');
  31 |     for (const user of users) {
  32 |       await login.login(user.username, user.password);
  33 | 
  34 |       // Check logout button visible after login
  35 |       await expect(page.locator('#logout2')).toBeVisible({ timeout: 10000 });
  36 | 
  37 |       // Logout to reset state
  38 |       await login.logout();
  39 |       await expect(page.locator('#login2')).toBeVisible({ timeout: 10000 });
  40 |     }
  41 |   });
  42 | 
  43 |   test('Login with multiple users from JSON', async ({ page }) => {
  44 |     const login = new LoginPage(page);
  45 |     await login.navigate();   // homepage load
  46 | 
  47 |     const users = readJSON('data-files/users.json');
  48 |     for (const user of users) {
  49 |       await login.login(user.username, user.password);
  50 | 
  51 |       // Check logout button visible after login
> 52 |       await expect(page.locator('#logout2')).toBeVisible({ timeout: 10000 });
     |                                              ^ Error: expect(locator).toBeVisible() failed
  53 | 
  54 |       // Logout to reset state
  55 |       await login.logout();
  56 |       await expect(page.locator('#login2')).toBeVisible({ timeout: 10000 });
  57 |     }
  58 |   });
  59 | 
  60 | });
