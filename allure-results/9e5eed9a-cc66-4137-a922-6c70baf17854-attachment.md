...
Call log:
  - waiting for locator('#tbodyid a.hrefch') to be visible
...
```ts
  ...
  11 |     // Wait for product links to load
  12 |     await this.page.waitForSelector('#tbodyid a.hrefch');
  13 |   }
  14 | 
  15 |   async addItemToCart(itemName) {
  16 |     await this.page.locator('#tbodyid a.hrefch', { hasText: itemName }).first().click();
  17 | 
...
> 26 |     await this.page.waitForSelector('#tbodyid a.hrefch');
     |                     ^ Error: page.waitForSelector: Test timeout of 30000ms exceeded.
  27 |   }
  28 | }
```